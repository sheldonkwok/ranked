...
import { heading, row } from "@/components/ui/surface";
...
function CoverCard({ game, onClick }: { game: ComparisonGame; onClick: () => void }) {
  return (
    <button
      ...
      className={row({
        className:
          "flex flex-1 flex-col items-center gap-3 border border-edge/45 p-4 text-center active:translate-y-0.5 mobile:p-2.5",
      })}
    >
      <CoverImage
  ...
  return (
    <ModalShell onCloseAction={onCloseAction} closeDisabled={closeDisabled}>
      <div className="flex flex-col items-center gap-1 text-center">
        <h2 className={heading({ className: "text-[12px]" })}>WHICH DID YOU LIKE MORE?</h2>
      ...
      <button type="button" onClick={onSkipAction} className={button({ variant: "ghost", className: "self-center" })}>
        TOO CLOSE — SKIP
      </button>
    </ModalShell>
  );
