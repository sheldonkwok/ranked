...
// Four flavors of pixel button — not a base+modifier system; each variant carries its own full padding/font/border set rather than layering onto a shared base.
export const button = cva("cursor-pointer whitespace-nowrap font-sans tracking-[1px] duration-100", {
  variants: {
    variant: {
      ...
      // Transparent 2px-outline action — EntryDialog's destructive buttons, muted until hover reveals the danger tint.
      outline:
...
// Bare icon CTA — the Plus/Share icons have no padding so their hit area is just the SVG box, below the 44px touch-target floor; `mobile:min-h-11 mobile:min-w-11` widens the tappable area without changing icon size, and `data-state="active"` marks icon buttons that are a mode switch (e.g. the /add Steam library toggle) rather than a one-shot action.
